...
import com.ternsip.soil.common.events.network.OnClientConnect;
import com.ternsip.soil.common.events.network.OnClientDisconnect;
    ...
    private void removeInactiveConnections() {
        getConnections().removeIf(connection -> {
            if (!connection.isActive()) {
                Soil.THREADS.getUniverseServer().networkEventReceiver.registerEvent(OnClientDisconnect.class, new OnClientDisconnect(connection));
                return true;
            }
            return false;
        });
