...
public class NetworkClient implements Threadable {
    ...
    private void establishConnection(Connection connection) {
        ...
        Soil.THREADS.client.eventReceiver.registerEvent(new OnConnectToServer(connection));
    }

    private void disconnect() {
        if (getConnection().isActive()) {
            getConnection().close();
            Soil.THREADS.client.eventReceiver.registerEvent(new OnDisconnectedFromServer());
